...
async function postWebhook(event: ModEventView, isReport: boolean) {
  ...
  if (event.subject.$type === "com.atproto.admin.defs#repoRef") {
    ...
    const subjectProfile = await agent.getProfile({ actor: event.subject.did });
    ...
    if (event.subject.uri.includes("app.bsky.feed.post")) {
      ...
      if (posts.data.posts.length > 0) {
        message.addField(
          "Post:",
          posts.data.posts[0].record.text ? posts.data.posts[0].record.text : ""
        );
      }
  ...
  message.addField("Comment:", event.event.comment ? event.event.comment : "");
